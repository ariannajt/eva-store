<template>
  <router-link :to="{ name: 'product-detail', params: { id: product.id } }" class="card">
    <div class="card__image">
      ...
      <span v-if="product.discount_active" class="card__badge card__badge--discount">
        -{{ product.discount_percent }}%
      </span>
      <span v-if="product.stock <= 0" class="card__badge card__badge--stock">Agotado</span>
    </div>
    <div class="card__body">
      ...
      <span v-if="product.discount_active" class="card__old-price">{{ formatMoney(product.price) }}</span>
    </div>
  </router-link>
</template>
